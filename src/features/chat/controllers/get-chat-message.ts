...
export class Get {
  public async conversationList(req: Request, res: Response): Promise<void> {
    ...
    if (cachedList.length) {
      ...
      list = await chatService.getUserConversationList(new mongoose.Schema.Types.ObjectId(req.currentUser!.userId));
    }
  ...
  public async messages(req: Request, res: Response): Promise<void> {
    ...
    if (cachedMessages.length) {
      ...
      messages = await chatService.getMessages(
        new mongoose.Schema.Types.ObjectId(req.currentUser!.userId),
        new mongoose.Schema.Types.ObjectId(receiverId),
        { createdAt: 1 }
      );
    }
